    ...
    <main className='min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8 px-4'>
      <div className='max-w-6xl mx-auto'>
        ...
        <div className='bg-white rounded-2xl shadow-xl p-8 mb-8'>
          <h1 className='text-4xl font-bold text-slate-800 mb-8'>Profile Settings</h1>
          ...
          <form onSubmit={handleSubmit} className='space-y-6'>
            <div className='flex flex-col items-center mb-6'>
              ...
              <img
                ...
                src={formData.avatar || (user && user.imageUrl)}
                alt='profile'
                className='rounded-full h-32 w-32 object-cover cursor-pointer border-4 border-slate-200 hover:border-slate-400 transition-all mb-4'
              />
              <p className='text-sm text-slate-600 mb-2'>Click to change profile picture</p>
              {fileUploadError && (
                <p className='text-red-600 text-sm'>Error: Image upload failed (max 2MB)</p>
              )}
              {filePerc > 0 && filePerc < 100 && (
                <p className='text-slate-600 text-sm'>Uploading {filePerc}%...</p>
              )}
              {filePerc === 100 && (
            ...
            <div className='flex gap-4 pt-4'>
              <button
                type='submit'
                disabled={loading}
                className='flex-1 bg-gradient-to-r from-slate-700 to-slate-800 text-white rounded-xl py-3 font-semibold hover:from-slate-800 hover:to-slate-900 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg'
              >
                {loading ? 'Updating...' : 'Update Profile'}
              </button>
              <Link
                to='/create-listing'
                className='flex items-center justify-center gap-2 px-6 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl font-semibold hover:from-green-700 hover:to-green-800 transition-all shadow-lg'
              >
                <FaPlus />
                Create Listing
              </Link>
          ...
          <div className='flex justify-between items-center mt-8 pt-8 border-t border-slate-200'>
            <button
              ...
              className='px-6 py-2 border border-slate-300 text-slate-700 rounded-xl font-medium hover:bg-slate-50 transition-all'
            >
              Sign Out
            </button>
            <button
              onClick={handleDeleteUser}
              className='px-6 py-2 bg-red-600 text-white rounded-xl font-medium hover:bg-red-700 transition-all'
            >
              Delete Account
            </button>
          </div>
